...
def create_plot(colormap): #create plot with color map
    ...
    sns.heatmap(vmin = 0.0, vmax = 1.0, data = data, cmap = colormap)
    return f
...
def init_TLFrame():
    ...
    global values
    values = {"1": "rocket", "2": "mako", "3": "Blues", "4": "YlOrBr",
            "5": "icefire"}
...
def init_SetupFrame():
    ...
    #choose file prompt
    tk.Label(SetupFrame, text = "Choose data file").grid(row = 0, column = 0, sticky = "w", **opts)

    #show file name
    global showfilename
    showfilename = tk.Label(SetupFrame, text = "No file chosen")
    showfilename.grid(row = 1, column = 1, **opts)

    choosefile = tk.Button(SetupFrame, text = "Browse", command = choose_file)
    choosefile.grid(row = 0, column = 1, **opts)

    tk.Label(SetupFrame, text = "Include 0s in calculation?").grid(row = 2, column = 0, sticky = "w", **opts)
